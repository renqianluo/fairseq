...
def main(args):
    ...
    # Load dataset splits
    load_dataset_splits(task, ['train', 'valid'], shuffle=False)
    
    if args.select_method == 'random':
        random_select(args, task.dataset(args.train_subset), seed=args.seed)
        return

    assert args.select_method == 'gradients'
    ...
    train_meter.start()
    cache = {}
    for e in range(args.select_epochs):
    #while lr > args.min_lr and epoch_itr.epoch < max_epoch and trainer.get_num_updates() < max_update:
        # train for one epoch
        train(args, trainer, task, epoch_itr)
        if args.select_average_gradients and e != args.select_epochs-1:
            validate_with_gradients(args, trainer, task, epoch_itr, cache)
    validate_with_gradients(args, trainer, task, epoch_itr, cache)
...
def validate_with_gradients(args, trainer, task, epoch_itr, cache):
    """Evaluate the model on the validation set(s) and return the losses."""
    valid_losses = []
    subset = 'train'
    # Initialize data iterator
    ...
    for split in splits:
        if split == 'train':
            ...
            for k in itertools.count():
                ...
                try:
                    task.load_dataset(split_k, combine=False, shuffle=False)
...
if __name__ == '__main__':
    parser = options.get_select_data_parser()
