...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        self.check_index(i)
        tensor_size = self.sizes[i] #self.sizes[self.dim_offsets[i]:self.dim_offsets[i + 1]]
...
class IndexedCachedDataset(IndexedDataset):
    ...
    def prefetch(self, indices):
        ...
        total_size = 0
        for i in indices:
            total_size += self.sizes[i] #self.data_offsets[i + 1] - self.data_offsets[i]
        self.cache = np.empty(total_size, dtype=self.dtype)
        ptx = 0
        self.cache_index.clear()
        for i in indices:
            self.cache_index[i] = ptx
            size = self.sizes[i] #self.data_offsets[i + 1] - self.data_offsets[i]
